...
public class BookTableGateway 
{
	...
	public ObservableList<AuthorBookModel> getAuthorsForBook(BookModel book) throws SQLException 
	{
		ObservableList<AuthorBookModel> authorBook = FXCollections.observableArrayList();
		
		//SELECT * FROM author_book INNER JOIN authordb on author_book.author_id = authordb.id WHERE author_book.book_id = ?
		//PreparedStatement st = conn.prepareStatement("SELECT * FROM author_book WHERE book_id = ?");
		PreparedStatement st = conn.prepareStatement("SELECT * FROM author_book INNER JOIN authordb on author_book.author_id = authordb.id WHERE author_book.book_id = ?");
		st.setInt(1, book.getId());
		ResultSet rs = st.executeQuery();
		
		while(rs.next())
		{
			AuthorBookModel authorInfo = new AuthorBookModel("default", 0.00);
			authorInfo.setAuthor(rs.getString("first_name") + " " + rs.getString("last_name"));
			authorInfo.setBookID(rs.getInt("book_id"));
			authorInfo.setAuthorID(rs.getInt("author_id"));
			authorInfo.setRoyalty(rs.getDouble("royalty"));
			authorBook.add(authorInfo);
		}
	...
	/*Commit New Royalties Value
	 * 
	 **/
	public void commitNewRValue(BookModel bookModel, Double newDouble, Double oldDouble) throws SQLException 
	{
		//System.out.println(bookModel.getId() + " " + newDouble + " " + oldDouble + " " + bookModel.getId());
		// TODO Auto-generated method stub
		PreparedStatement st = conn.prepareStatement("UPDATE author_book SET royalty = ? WHERE book_id = ? and royalty = cast(? as decimal(6,5))");
		st.setDouble(1, newDouble);
		st.setInt(2, bookModel.getId());
		st.setDouble(3, oldDouble);		
		st.executeUpdate();
		...
		st.setString(2, "Author Royalty Changed from " + oldDouble + " to " + newDouble);
		st.executeUpdate();
		
	}

	/*Delete Author/Royalties
	 * 
	 * */
	public void deleteAuthorRoyal(int authorID, int bookID, AuthorBookModel authorBookModel) throws SQLException 
	{
		// TODO Auto-generated method stub
		...
		st = conn.prepareStatement("INSERT INTO book_audit_trail(book_id, entry_msg) values(?, ?)");
		st.setInt(1, bookID);
		st.setString(2, "Author " + authorBookModel.getAuthor() + " Deleted from book");
		st.executeUpdate();
	}



	public void CreateNewAuthorRoyalty(AuthorModel author, String royalties, String dob, BookModel bookModel) throws SQLException 
	{
		//Create New Author
		PreparedStatement st = conn.prepareStatement("INSERT INTO authordb (first_name, last_name, dob, gender, web_site) VALUES (?, ?, ?, ?, ?)");
		st.setString(1, author.getFName());
		st.setString(2, author.getLName());
		st.setString(3, dob);
		st.setString(4, author.getGender());
		st.setString(5, author.getWeb());
		st.executeUpdate();
		
		//Get generated Author ID for First and Last name (should match if unique)
		//TODO make entries unique on first and last name
		st = conn.prepareStatement("SELECT id FROM authordb WHERE first_name = ? and last_name = ?");
		st.setString(1, author.getFName());
		st.setString(2, author.getLName());
		ResultSet rs = st.executeQuery();
		while(rs.next())
		...
		st.setInt(1, author.getaID());
		st.setInt(2, bookModel.getId());
		st.setString(3, royalties);
		st.executeUpdate();
		
		//Create new Author Addition Audit Trail Entry
		//Write the audit entry
		st = conn.prepareStatement("INSERT INTO book_audit_trail(book_id, entry_msg) values(?, ?)");
		st.setInt(1, bookModel.getId());
		st.setString(2, "Author was added too book: " + author.getFName() + " " + author.getLName());
		st.executeUpdate();
